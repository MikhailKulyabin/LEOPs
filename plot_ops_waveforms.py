...
def load_ops_waveforms():
    """Load OPs waveforms from JSON files, grouped by (protocol, flash_bin, group)."""
    ...
    for fname in sorted(os.listdir(JSON_DIR)):
        ...
        for rec in pdata["recordings"]:
            ...
            op = rec.get("op_waveform")
            if not op or not op.get("time_ms") or not op.get("amplitude_uv"):
                continue

            t = np.array(op["time_ms"], dtype=np.float64)
            a = np.array(op["amplitude_uv"], dtype=np.float64)

            if len(t) == 0 or np.any(np.isnan(t)) or np.any(np.isnan(a)):
                continue
...
def make_ops_9step_figure(data, out_path):
...
def make_ops_2step_figure(data, out_path):
...
def main():
    print("Loading OPs waveforms from JSON files...")
    data = load_ops_waveforms()

    total = sum(len(v) for v in data.values())
    combos = len(set(k[:2] for k in data.keys()))
    print(f"Loaded {total} OPs waveforms across {combos} protocol x flash "
          f"combinations.\n")

    print("Generating OPs figures...")
    make_ops_9step_figure(data, os.path.join(OUT_DIR, "figure_ops_9step.pdf"))
    make_ops_2step_figure(data, os.path.join(OUT_DIR, "figure_ops_2step.pdf"))
